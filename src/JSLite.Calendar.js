...
$.calendar=function(elm,json,callback){
	...
	this.isWork=function(dates){//是否为工作日
		...
		var cls="";
		this.workdate.forEach(function(obj,idx){
	...
	this.creatMonth=function(y,m,d){
		...
		dayFull().map(function(num){
			if(num>week&&num<solarMonth[parseInt(m-1)]+week+1){
				...
				var a = y+"/"+m+"/"+d;
				elmD.append($(html).off('click').on('click',function(){
	...
	this.time=function(_format,_str){
		this.format = _format;
		var time_str = '';
		if(_str && _str === 'default')time_str = format(new Date(),_format);
		if(_str && _str !== 'default' && typeof(_str) === 'string') time_str = format(new Date(_str),_format);
	...
	this.hide=function(){
		...
		this.calBoxs.css({"position":"absolute","display":"none","z-index":"999"}).prev().click(function(ev){
			$(this).next().show().data('hide',1);
		})
		$(document).on("click",function(ev,y){
			var $box = $(ev.target).next()
			var num = $box.data('hide')

			if(!num&& $(ev.target).parents('.JSLiteCalenbar').length===0
					&&!$(ev.target).is('.lbtn')
					&&!$(ev.target).is('.rbtn')){
				$('.JSLiteCalenbar').each(function(){
